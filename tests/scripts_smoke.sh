...
(async () => {
  ...
  resetCalls();
  state.primaryWindow = state.primary;
  await runSecondInstance(["--new-window"]);
  assert(state.createFreshLocalWindowCalls.length === 1 && state.createFreshLocalWindowCalls[0] === "/", "second-instance --new-window should create a fresh main window");
  assert(state.focusCalls.length === 1 && state.focusCalls[0] === "created", "second-instance --new-window should focus the fresh window");
